alter table private.customer_hard_delete_jobs
  add column if not exists ocr_batch_ids uuid[] not null default '{}'::uuid[];
...
begin
  ...
  where exists (select 1 from public.ocr_results r where r.batch_id=b.id and r.created_customer_id=any(v_ids))
    and not exists (select 1 from public.ocr_results r where r.batch_id=b.id and (r.created_customer_id is null or not (r.created_customer_id=any(v_ids))));
...
begin
  ...
  delete from public.ocr_results where created_customer_id=any(v_job.customer_ids);
