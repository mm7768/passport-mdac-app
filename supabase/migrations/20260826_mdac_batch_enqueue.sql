-- Atomically enqueue an MDAC registration batch from the authenticated Flutter client.
-- This function stores a point-in-time customer snapshot for the Worker.
...
begin
  ...
  insert into public.automation_batches (
    ...
    p_entry_date,
    p_exit_date,
    coalesce(nullif(trim(p_note), ''), '已保存客户快照，等待 MDAC fill-preview Worker；禁止提交')
