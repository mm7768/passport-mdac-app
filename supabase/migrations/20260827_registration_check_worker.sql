-- Check Registration is implemented from scratch.
-- No CAPTCHA solving, no form submission, no credentials in this migration.
-- PIN values are never copied into customer_snapshot, which is client-readable.

alter table public.registration_checks
...
create index if not exists automation_batches_registration_check_queue_idx
  on public.automation_batches (task_type, status, created_at)
  where task_type = 'REGISTRATION_CHECK';

create index if not exists registration_checks_status_idx
  on public.registration_checks (result_status, updated_at desc);

comment on column public.registration_checks.screenshot_path is
  'Private Storage path for a minimal Check Registration review screenshot; never a public URL.';
comment on column public.registration_checks.challenge_type is
  'Detected official challenge type such as CAPTCHA_SLIDER; null when no challenge was observed.';
comment on column public.registration_checks.submitted is
  'Always false in the current Check Registration worker; retained as an explicit safety field.';
comment on column public.registration_checks.result_confirmed is
  'True only after a future authorized result-confirmation flow; current worker always writes false.';

create or replace function public.create_registration_check_batch(
  p_items jsonb,
...
begin
  ...
  if v_item_count < 1 or v_item_count > 200 then
    raise exception 'Registration Check batch must contain between 1 and 200 items';
  end if;

  for v_item in select value from jsonb_array_elements(p_items)
  loop
    if jsonb_typeof(v_item) <> 'object' then
      raise exception 'each Registration Check item must be a JSON object';
    end if;
    ...
    if v_customer_id is null or v_snapshot is null or jsonb_typeof(v_snapshot) <> 'object' then
      raise exception 'each Registration Check item requires customer_id and customer_snapshot';
    end if;
    if nullif(trim(v_snapshot ->> 'passport_number'), '') is null then
      raise exception 'each Registration Check item requires passport_number';
    end if;
    if nullif(trim(v_snapshot ->> 'nationality'), '') is null then
      raise exception 'each Registration Check item requires nationality';
    end if;
    if not exists (
      select 1
  ...
  insert into public.automation_batches (
    ...
    0,
    0,
    coalesce(nullif(trim(p_note), ''), '已保存 Check Registration 客户快照，等待查询 Worker；不保存 PIN 到快照')
...
create or replace function public.claim_registration_check_batch(
  p_worker_id text,
...
begin
  ...
  select b.* into v_batch
   ...
   where b.task_type = 'REGISTRATION_CHECK'
  ...
  update public.automation_items
     set status = 'QUEUED',
         ...
         error_message = 'Previous Registration Check Worker lease expired; item returned to queue',
  ...
  update public.automation_batches
     set status = 'CLAIMED',
         ...
         note = coalesce(note, '') || ' | Registration Check Worker 已领取；不自动处理 CAPTCHA'
   where id = v_batch.id;
...
create or replace function public.claim_registration_check_item(
  p_batch_id uuid,
  p_worker_id text,
  p_lease_seconds integer default 900,
  p_max_attempts integer default 5
...
begin
  ...
  select i.* into v_item
   ...
   where i.batch_id = p_batch_id
     ...
     and b.task_type = 'REGISTRATION_CHECK'
...
create or replace function public.get_registration_check_runtime_input(
  p_item_id uuid,
  p_worker_id text
)
returns table (
...
create or replace function public.heartbeat_registration_check(
  p_worker_id text,
  ...
  p_version text default 'registration-check-1'
)
returns void
language plpgsql
security definer
...
begin
  ...
  insert into public.worker_heartbeats (
    ...
    coalesce(nullif(trim(p_version), ''), 'registration-check-1'),
    p_status,
...
create or replace function public.finish_registration_check_item(
  p_item_id uuid,
  p_worker_id text,
  ...
  update public.automation_batches b
     set locked_by = null,
         ...
         note = coalesce(b.note, '') || case
           ...
           else ' | Check Registration 结果待确认；未提交'
         end
...
revoke execute on function public.create_registration_check_batch(jsonb, text) from public, anon;
grant execute on function public.create_registration_check_batch(jsonb, text) to authenticated;

revoke execute on function public.claim_registration_check_batch(text, integer, integer) from public, anon, authenticated;
revoke execute on function public.claim_registration_check_item(uuid, text, integer, integer) from public, anon, authenticated;
revoke execute on function public.get_registration_check_runtime_input(uuid, text) from public, anon, authenticated;
revoke execute on function public.heartbeat_registration_check(text, uuid, uuid, integer, public.worker_status, text, text) from public, anon, authenticated;
...
grant execute on function public.claim_registration_check_batch(text, integer, integer) to service_role;
grant execute on function public.claim_registration_check_item(uuid, text, integer, integer) to service_role;
grant execute on function public.get_registration_check_runtime_input(uuid, text) to service_role;
grant execute on function public.heartbeat_registration_check(text, uuid, uuid, integer, public.worker_status, text, text) to service_role;
...
comment on function public.get_registration_check_runtime_input(uuid, text) is
...
comment on function public.finish_registration_check_item(uuid, text, public.check_status, text, jsonb, text, text, boolean, boolean, boolean, integer, text, text) is
  'Atomic Check Registration review writeback. Current worker cannot confirm results, submit, or solve CAPTCHA.';
